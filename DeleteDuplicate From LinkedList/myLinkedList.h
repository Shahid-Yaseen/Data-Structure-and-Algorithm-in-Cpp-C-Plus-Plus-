...
void myLinkedList<T>::deleteDuplicate(){
    Node<T>* movement = myLinkedList<T>::head;
  	for(Node<T>* i = movement->next;movement->next!=nullptr;i = i->next)
	{
		if (i== nullptr ){
		 movement=movement->next;
		 if(movement->next==nullptr)
		    break;
		    i=movement->next;
		}
		if(i->data==movement->data){
    	    deleteValue(i->data);
		    movement=myLinkedList<T>::head;
		    i=myLinkedList<T>::head;
     	}
